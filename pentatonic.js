...
const pentatonic = (
	...
	if (highestNote > keyLimit) {
		...
		notes = notes.reduce((array, count) => {
			return [
				...array,
				Math.round(rangeMap(count, 0, highestNote, 0, keyLimit)),
			];
		}, []);
	}
	...
	let instrument = sharedAudioContext.createOscillator();
	let amplifier = sharedAudioContext.createGain();
	...
	let periodicWave = sharedAudioContext.createPeriodicWave(real, imag);
	...
	amplifier.connect(sharedAudioContext.destination);
	// Start playing notes!
	playNotes();
